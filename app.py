...
# --- ROBUST JOB VIEW ROUTE START ---
from flask import abort, jsonify
@app.route('/jobs/<int:job_id>')
@login_required
def view_job(job_id):
    # Load job and build a safe dict for the template to avoid attribute errors
    job_obj = Job.query.get(job_id)
    if not job_obj:
        abort(404)
    # Build a plain dict with safe fallbacks
    def safe_attr(o, name):
        try:
            return getattr(o, name)
        except Exception:
            return None
    job = {}
    job['id'] = safe_attr(job_obj, 'id')
    job['job_number'] = safe_attr(job_obj, 'job_number') or ''
    job['title'] = safe_attr(job_obj, 'title') or ''
    job['client_name'] = safe_attr(job_obj, 'client_name') or ''
    job['address'] = safe_attr(job_obj, 'address') or ''
    job['status'] = safe_attr(job_obj, 'status') or ''
    job['service_type'] = safe_attr(job_obj, 'service_type') or ''
    created_at = safe_attr(job_obj, 'created_at')
    job['created_at'] = created_at.isoformat() if created_at else None

    # Photos: try common relationship names and fall back to empty list
    photos = []
    for rel in ('photos','job_photos','images'):
        try:
            coll = getattr(job_obj, rel)
            if coll is not None:
                for p in coll:
                    try:
                        photos.append({
    ...
    # Checklists: try common names
    checklists = []
    for rel in ('checklists','job_checklists','lists'):
        try:
            coll = getattr(job_obj, rel)
            if coll is not None:
                for c in coll:
    ...
    # Assigned and admin names (if relationships exist)
    assigned_to = None
    admin = None
    for rel in ('assigned_to','assignee','user'):
        try:
            a = getattr(job_obj, rel)
            if a is not None:
                assigned_to = getattr(a,'name', None) or getattr(a,'email', None)
                break
        except Exception:
            continue
    for rel in ('admin','created_by','owner'):
        try:
            a = getattr(job_obj, rel)
            if a is not None:
                admin = getattr(a,'name', None) or getattr(a,'email', None)
                break
        except Exception:
            continue
    ...
    return render_template('view_job.html', job=job, is_admin=is_admin())
# --- ROBUST JOB VIEW ROUTE END ---
