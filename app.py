...
class User(db.Model, UserMixin):
    ...
    role = db.Column(db.String(50), default="contractor")  # "admin" or "contractor"
...
class Job(db.Model):
    ...
    address = db.Column(db.String(255), nullable=False)  # used for Google Maps
...
class ChecklistTemplate(db.Model):
    """
    Admin-defined checklist templates per service type.
    Example: 'Water Damage', 'Fire Damage', 'Packout', etc.
    """
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(255), nullable=False)
...
class JobChecklist(db.Model):
    """
    Actual checklist attached to a job, based on a template.
    """
    id = db.Column(db.Integer, primary_key=True)
...
class Photo(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    job_id = db.Column(db.Integer, db.ForeignKey("job.id"), nullable=False)
    room = db.Column(db.String(100), nullable=True)  # e.g., "Kitchen", "Bedroom 1"
    category = db.Column(db.String(100), nullable=True)  # e.g., "Before", "During", "After"
...
class Contract(db.Model):
    ...
    signature_image = db.Column(db.String(255), nullable=True)  # path to signature PNG
...
>>>>>>> d3d21ed3dd6840fb58db49c5062c440449d3d262
...
def admin_checklists():
    if request.method == "POST":
        ...
        items_raw = request.form.get("items")  # newline-separated
        ...
        for line in items_raw.splitlines():
            ...
            if not line:
                continue
            item = ChecklistItemTemplate(template_id=template.id, description=line)
            db.session.add(item)
...
# -----------------------------------------------------------------------------
# JOBS: CREATE / EDIT / STATUS / DELETE
# -----------------------------------------------------------------------------
...
def create_job():
    ...
    if request.method == "POST":
        ...
        # Attach checklist template if selected
...
# -----------------------------------------------------------------------------
# JOB CHECKLIST ITEMS: COMPLETE / UNCOMPLETE
# -----------------------------------------------------------------------------
...
def toggle_checklist_item(item_id):
    ...
    # Only admin or assigned contractor can toggle
    ...
    return redirect(url_for("view_job", job_id=job.id))
# -----------------------------------------------------------------------------
# PHOTOS (CompanyCam-style per job/room/category)
# -----------------------------------------------------------------------------
...
def upload_photo(job_id):
    ...
    # Only admin or assigned contractor
    if not (is_admin() or current_user.id in [job.assigned_to_id, job.admin_id]):
        flash("Not allowed.", "danger")
        return redirect(url_for("view_job", job_id=job.id))
...
def sign_contract(contract_id):
    ...
    if request.method == "POST":
        ...
        signature_data_url = request.form.get("signature_data_url")  # base64 PNG from canvas
    ...
    return render_template("sign_contract.html", contract=contract, job=job)


# -----------------------------------------------------------------------------
# PDF REPORT (simple stub)
# -----------------------------------------------------------------------------
...
def job_report(job_id):
    job = Job.query.get_or_404(job_id)
    report_filename = f"job_{job.id}_report.pdf"
    report_path = os.path.join(UPLOAD_FOLDER, report_filename)

    c = canvas.Canvas(report_path)
    c.setFont("Helvetica", 14)
    c.drawString(50, 800, f"Job Report: {job.job_number} - {job.title}")
    c.setFont("Helvetica", 10)
    c.drawString(50, 780, f"Client: {job.client_name}")
    c.drawString(50, 765, f"Address: {job.address}")
    c.drawString(50, 750, f"Status: {job.status}")
    c.drawString(50, 735, f"Service Type: {job.service_type or ''}")
    c.drawString(50, 720, f"Created: {job.created_at}")
    c.drawString(50, 705, f"Updated: {job.updated_at}")

    c.drawString(50, 680, "Checklists:")
    y = 665
    for checklist in job.checklists:
        c.drawString(60, y, f"- {checklist.template_name} ({checklist.service_type})")
        y -= 15
        for item in checklist.items:
            status = "✔" if item.completed else "✘"
            c.drawString(80, y, f"{status} {item.description}")
            y -= 15
        y -= 10
    ...
    for contract in job.contracts:
        status = "Signed" if contract.signed else "Not signed"
        c.drawString(60, y, f"- {contract.filename} ({status})")
        y -= 15
...
# -----------------------------------------------------------------------------
# SIMPLE JSON API (optional for future mobile/PWA)
# -----------------------------------------------------------------------------
